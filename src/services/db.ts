...
export function calculateEloTS(rA: number, rB: number, setsA: number, setsB: number, k: number = 32) {
...
export const dbService = {
  // --- AUTH SERVICES ---
  async getCurrentUser(): Promise<{ id: string; email: string; username: string; display_name: string } | null> {
  ...
  async signup(email: string, password: string,username: string, displayName: string, age: number, playerType: 'amateur' | 'competitive' | 'student'): Promise<Profile> {
    if (isSupabaseConfigured && supabase) {
  ...
  async login(email: string, password: string): Promise<Profile> {
    if (isSupabaseConfigured && supabase) {
      ...
      if (!profile) {
        throw new Error('Utente non trovato (inserisci l\'email con l\'username dei giocatori di prova, es: marco_topspin@rankpong.local)');
      }
  ...
  async getMatches(): Promise<MatchWithSets[]> {
    if (isSupabaseConfigured && supabase) {
      ...
      return matchesData.map((match: any) => ({
        ...match,
        sets: setsData.filter((set: any) => set.match_id === match.id)
          .sort((a, b) => a.set_number - b.set_number)
      })) as MatchWithSets[];
    } else {
      const matches = JSON.parse(localStorage.getItem('rp_matches') || '[]') as MatchWithSets[];
      const profiles = JSON.parse(localStorage.getItem('rp_profiles') || '[]') as Profile[];
