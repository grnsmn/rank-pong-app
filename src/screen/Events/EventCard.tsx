...
import { Swords, Clock, Calendar, Trophy, Zap } from 'lucide-react'
		...
		>
			<div className="px-4 pt-3.5 pb-3">
				<div className="flex justify-between items-center gap-2 pb-2 mb-2.5 border-b border-slate-800">
					<span className="flex items-center gap-1.5 flex-wrap">
						...
						<span
							className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wide ${statusBadge}`}
						>
							{statusLabel}
						</span>
					</span>
				...
				<div className="text-[15px] font-bold text-white mb-0.5">{event.name}</div>
				...
				{event.status === 'completed' && winnerName && (
					<div className="flex items-center gap-2.5 px-3 py-2 rounded-xl bg-slate-950/60">
						...
						<span className="flex-1 text-xs font-bold text-yellow-400 truncate">
							{winnerName}
						</span>
						{myFinalRank != null && (
						...
						{myPoints != null && (
							<span className="text-xs font-extrabold text-slate-300 shrink-0">
								+{myPoints}
							</span>
						)}
					</div>
			...
			{projection && (
				<div className="flex items-center gap-2.5 px-4 py-2.5 bg-success/10 border-t border-slate-800/60">
					...
					<span
						className={`text-[15px] font-black shrink-0 ${deltaColor(
