...
export const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'leaderboard' | 'matches' | 'new-match' | 'profile'>('leaderboard');
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-950 text-white">
        ...
        <h2 className="text-sm font-bold tracking-widest uppercase text-slate-400">Caricamento RankPong...</h2>
      </div>
    );
  }
      ...
      {!isSupabaseConfigured && !hideBanner && (
        <div className="bg-gradient-to-r from-amber-600 to-yellow-600 text-white text-xs px-4 py-2.5 flex items-center justify-between shadow-md shrink-0 z-50">
          <div className="flex items-center gap-2">
            ...
            <span>
              <strong>Modalità Demo attiva:</strong> i dati sono salvati in locale. Inserisci le chiavi in un file <code>.env</code> per connettere Supabase.
            </span>
          </div>
          <button 
        ...
        <div className="grid grid-cols-4 items-center justify-around">
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center py-1 transition-colors ${
              activeTab === 'leaderboard' ? 'text-primary' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <Trophy className={`w-5 h-5 ${activeTab === 'leaderboard' ? 'fill-primary/10' : ''}`} />
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center py-1 relative transition-colors ${
              activeTab === 'matches' ? 'text-primary' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center py-1 transition-colors ${
              activeTab === 'new-match' ? 'text-primary' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <PlusCircle className={`w-5 h-5 ${activeTab === 'new-match' ? 'fill-primary/10' : ''}`} />
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center py-1 transition-colors ${
              activeTab === 'profile' ? 'text-primary' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <User className={`w-5 h-5 ${activeTab === 'profile' ? 'fill-primary/10' : ''}`} />
